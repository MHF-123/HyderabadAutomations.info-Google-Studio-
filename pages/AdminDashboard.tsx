...
const ManageIndustries: React.FC<{ industries: Industry[], setIndustries: Function, editingId: string | null, setEditingId: Function }> = ({ industries, setIndustries, editingId, setEditingId }) => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>, formType: 'new' | 'edit') => {
        ...
        if (formType === 'new') {
            setNewIndustry((prev: Omit<Industry, 'id'>) => ({ ...prev, [name]: value }));
        } else {
            setEditingIndustry((prev: Industry | null) => prev ? { ...prev, [name]: value } : null);
        }
    };
    ...
    const handleArrayInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>, formType: 'new' | 'edit') => {
        const { name, value } = e.target;
        if (formType === 'new') {
            setNewIndustry((prev: Omit<Industry, 'id'>) => ({ ...prev, [name]: value.split('\n') }));
        } else {
            setEditingIndustry((prev: Industry | null) => prev ? { ...prev, [name]: value.split('\n') } : null);
        }
    };
    ...
    const handleImageFileChange = (e: React.ChangeEvent<HTMLInputElement>, formType: 'new' | 'edit') => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                if (formType === 'new') {
                    setNewIndustry((prev: Omit<Industry, 'id'>) => ({ ...prev, image: reader.result as string }));
                } else {
                    setEditingIndustry((prev: Industry | null) => prev ? { ...prev, image: reader.result as string } : null);
                }
            };
            reader.readAsDataURL(file);
        }
     ...
     const handleImageUrlChange = (e: React.ChangeEvent<HTMLInputElement>, formType: 'new' | 'edit') => {
        ...
        if (formType === 'new') {
            setNewIndustry((prev: Omit<Industry, 'id'>) => ({ ...prev, image: value }));
        } else {
            setEditingIndustry((prev: Industry | null) => prev ? { ...prev, image: value } : null);
        }
    };
